...
def etl_camunda_database(query_path):
    ...
    firmas_camunda_pipeline = Pipeline([
        ('extract_data', dbm.DataExtractorCamunda(database='CAMUNDA', query_path=query_path, dtypes_variables=column_types, save_request=True, float_columns=float_columns)),
...
def etl_portal_database(query_path):
    ...
    firmas_portal_pipeline = Pipeline([
        ('extract_data', dbm.DataExtractorPortal(database='Portal', query_path=query_path, dtypes_variables=column_types, float_columns=float_columns, save_request=True)),
        ...
        ('Fill the column values with other column when is NA', fgp.FillNAValues('fecha_caducidad', 'fecha_caducidad_mod')),
        ...
        ('Add Column ', fgp.AgregarColumnaValor(column_name='estado_tramite', value='Finalización de Trámite')), # Agregada puesto que todos los registros extraidos fueron finalizados
    ])
...
def database_concat_portal_camunda(query_camunda, query_portal):
    ...
    concatenated_pipelines = Pipeline([
        ...
        ('Actualizacion de Date FACT por fecha Init Tramite en campos vacios', fgp.FillNAValues('fecha_factura', 'fecha_inicio_tramite')),
        ...
        ('Indicar pediodo de Renovacion', sf.VerificarPeriodoRenovacion('fecha_factura', 'fecha_caducidad', 'key', 'producto', 'vigencia')),
...
def ProcesosPendientesCamunda():
    ...
    procesos_pendientes_pipeline = Pipeline([
        ('extract_data', dbm.DataExtractorCamunda(database='CAMUNDA', query_path=query_path)),
        ('Count the amount of null Values', fgp.CountNullValuesRow('nulls_count')),
        ('delete_duplicates', fgp.DeleteDuplicateEntries(column='id_tramite', nulls_count='nulls_count')),
